...
flipDeltas = [-4, -3, -2, -1, 0, 1, 2, 3, 4]  # Example T1 values for pyruvate
noise_levels = [0.05] * len(flipDeltas)  # Keep noise levels consistent with T1 variations

# cov shape: (n_noise_levels, model_type(0=NN,1=Traditional), param_index(kPL=0,kVE=1,vB=2))
cov = np.zeros((len(noise_levels), 2, 3))
flips = np.zeros((len(noise_levels), 2))
...
for sim_idx, delta in enumerate(flipDeltas):
    r2_kpl = 0
    r2_kve = 0
    r2_vb  = 0

    noise = 0.05 
    generator = TwoCompartmentHPDataGenerator(time_points=time_points, flip_angle_pyr_deg=DEFAULT_VFA_SCHEDULE + delta, 
                                              flip_angle_lac_deg=DEFAULT_VFA_SCHEDULE + delta)
    X, y = generator.generate_dataset(n_samples=n_samples, noise_std=noise)
    X_raw = X.copy()  # raw data for vb
    X_norm = X 
    # 3. Flatten inputs
    ...
    flips[sim_idx, 0] = delta
    flips[sim_idx, 1] = delta
    ...
    with torch.no_grad():
        ...
        plot_true_vs_pred(y_true, y_pred, f"{delta}__{delta}_Neural Network")
    ...
    from datetime import datetime
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    fname = os.path.join(run_dir, f'NeuralNetwork_FlipDelta_{int(delta)}_{int(delta)}')
    ...
    for p_idx in range(3):
        ...
        if np.isfinite(mean_p) and np.abs(mean_p) > eps:
            cov[sim_idx, 0, p_idx] = std_p / np.abs(mean_p)
        else:
            cov[sim_idx, 0, p_idx] = np.nan
    # Store mean NN predictions (per-parameter) for this noise level
    if y_pred.shape[1] >= 3:
        mean_pred_nn[sim_idx, :] = np.nanmean(y_pred, axis=0)[:3]
    else:
        tmp = np.nanmean(y_pred, axis=0)
        mean_pred_nn[sim_idx, :tmp.shape[0]] = tmp
    ...
    for p_idx in range(3):
        ...
        if den > 0:
            r2_nn[sim_idx, p_idx] = 1 - num/den
        else:
            r2_nn[sim_idx, p_idx] = np.nan
    ...
    try:
        ...
        if n_samples_local > 1:
            ...
            r2_nn_std[sim_idx, :] = np.nanstd(r2b, axis=0)
    except Exception:
        pass
    ...
    fname = os.path.join(run_dir, f'TraditionalFitOfGeneratorData_R1_PL_{int(delta)}_{int(delta)}')
    ...
    if y_pred_fit.ndim == 2:
        for p_idx in range(min(3, y_pred_fit.shape[1])):
            ...
            den = np.nansum((y_true[:, p_idx] - np.nanmean(y_true[:, p_idx]))**2)
            if den > 0:
                r2_tr[sim_idx, p_idx] = 1 - num/den
            else:
                r2_tr[sim_idx, p_idx] = np.nan
            ...
            for b in range(B):
                ...
                for p_idx in range(min(3, y_pred_fit.shape[1])):
                    ...
                    if den_b > 0 and y_pf_b.shape[0] > 0:
                        ...
                        r2b[b, p_idx] = 1 - num_b/den_b
            r2_tr_std[sim_idx, :] = np.nanstd(r2b, axis=0)
    except Exception:
        pass

    
## Save flips and per-parameter CoV to Excel file
# Expand cov into separate 1-D columns so pandas DataFrame is happy
data_dict = {
    'Noise_Level': noise_levels,
    'Flip_Pyruvate': flips[:, 0],
    'Flip_Lactate': flips[:, 1],
...
plt.plot(flips[:, 0], cov[:, 0, 0], label='NN CoV (kPL)', marker='o', color='blue')
plt.plot(flips[:, 0], cov[:, 1, 0], label='NLLS CoV (kPL)', marker='x', color='orange')
plt.xlabel('Flip Delta Pyruvate (Flip Delta)')
plt.ylabel('Coefficient of Variation (kPL)')
plt.title('Coefficient of Variation (kPL) vs Flip Delta Pyruvate (degs)')
plt.legend()
...
for p_idx, pname in enumerate(params):
    ...
    plt.xlabel('Pyruvate Flip Delta (degs)')
    plt.ylabel('Coefficient of Determination (R²)')
    plt.title(f'Pyruvate Flip Delta vs R² ({pname})')
    plt.legend()
    ax.set_ylim(-0.5, 1.05)
    ax.grid(True, which='major', alpha=0.7)
    ax.grid(True, which='minor', alpha=0.3, linestyle=':')
    plt.tight_layout()
    plt.savefig(os.path.join(run_dir, f'FlipDeltaP_vs_R2_{pname}.png'), dpi=300, bbox_inches='tight')
    plt.close()
...
ax1.set_title('Pyruvate Flip Delta vs R² (all parameters)')
# ax1.legend(ncol=2)Fl
...
plt.tight_layout()
plt.savefig(os.path.join(run_dir, 'FlipDelta_vs_R2_all_params.png'), dpi=300, bbox_inches='tight')
plt.close()
