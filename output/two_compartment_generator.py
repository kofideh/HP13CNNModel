...
from fit_two_compartment import rf_loss  # noqa: F401
...
class TwoCompartmentHPDataGenerator:
    ...
    def generate_dataset(self, n_samples=1000, noise_std=0.05):
        ...
        for _ in range(n_samples):
            ...
            S_pyr_noisy = result['S_pyr'] + np.random.normal(0.0, noise_std, size=len(result['S_pyr']))
            S_lac_noisy = result['S_lac'] + np.random.normal(0.0, noise_std, size=len(result['S_lac']))
