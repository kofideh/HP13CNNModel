...
run_dir = os.path.join(output_dir, "wts_sim_16point_VFA")
...
generator = TwoCompartmentHPDataGenerator(time_points=time_points, TR=SCAN_TR, flip_angle_pyr_deg=PYR_FA_SCHEDULE, flip_angle_lac_deg=LAC_FA_SCHEDULE)
...
input_dim_norm = NUM_TIMEPOINTS*2
...
for p_idx, pname in enumerate(params):
    ...
    plt.xlabel('Pyruvate SNR')
    plt.ylabel('Coefficient of Determination (R²)')
    plt.title(f'Pyruvate SNR vs R² ({pname})')
    plt.legend()
...
for p_idx, pname in enumerate(params):
    ...
    plt.plot(x, r2_tr[:, p_idx], label=f'NLLS R² {pname}', marker=markers['NLLS'], color=colors[pname], linestyle='--')
...
plt.xlabel('Pyruvate SNR')
plt.ylabel('Coefficient of Determination (R²)')
plt.title('Pyruvate SNR vs R² (all parameters)')
plt.legend(ncol=2)
ax.set_ylim(-0.5, 1.05)
ax.grid(True, which='major', alpha=0.7)
ax.grid(True, which='minor', alpha=0.3, linestyle=':')
...
for ax_idx, ax in enumerate(axes):
    ...
    for p_idx, pname in enumerate(params):
        # NN
        ax.plot(x, r2_nn[:, p_idx], label=f'NN R² {pname}', marker=markers['NN'], color=colors[pname], linestyle='-')
        # Traditional
        ax.plot(x, r2_tr[:, p_idx], label=f'TR R² {pname}', marker=markers['TR'], color=colors[pname], linestyle='--')
        # Shaded bands (if available)
        if np.any(np.isfinite(r2_nn_std[:, p_idx])):
            ax.fill_between(x, r2_nn[:, p_idx] - r2_nn_std[:, p_idx], r2_nn[:, p_idx] + r2_nn_std[:, p_idx],
                            color=colors[pname], alpha=0.12)
        if np.any(np.isfinite(r2_tr_std[:, p_idx])):
            ax.fill_between(x, r2_tr[:, p_idx] - r2_tr_std[:, p_idx], r2_tr[:, p_idx] + r2_tr_std[:, p_idx],
                            color=colors[pname], alpha=0.06)
